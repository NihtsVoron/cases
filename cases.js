...
let RussianName = function(lastName, firstName, middleName, sex, rules) {
    if (typeof firstName == 'undefined') {
        let m = lastName.match(/^\s*(\S+)(\s+(\S+)(\s+(\S+))?)?\s*$/);
        if(!m) throw "Cannot parse supplied name";
        if(m[4] && m[2].match(/(ич|на)$/) && !m[4].match(/(ич|на)$/)) {
...
RussianName.prototype = {
    ...
    lastName: function(cases) {
        return RussianNameProcessor.word(this.ln, this.sex, 'lastName', cases);
    },
    firstName: function(cases) {
        return RussianNameProcessor.word(this.fn, this.sex, 'firstName', cases);
    },
    middleName: function(cases) {
        return RussianNameProcessor.word(this.mn, this.sex, 'middleName', cases);
    }
}